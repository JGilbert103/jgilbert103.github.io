...
function addEntry(blog) {
  ...
  postDate.textContent = new Date(blog.date).toLocaleDateString();
  postDate.classList.add('date');
  entry.appendChild(postDate);
  
  var postContent = document.createElement('p');
  postContent.textContent = blog.content;
  entry.appendChild(postContent);

  var delEntry = document.createElement('button');
  delEntry.textContent = "[x]";
  delEntry.classList.add('delete-btn');
  title.appendChild(delEntry);
}
