...
function initGame() {
    ...
    if (selectedCategory) {
        const categoryTitle = selectedCategory.getAttribute('data-category');
...
function processQuestions(data) {
    data.results.forEach(item => {
        ...
        const correctAnswerIndex = Math.floor(Math.random() * (item.incorrect_answers.length + 1));
        question.answers.splice(correctAnswerIndex, 0, item.correct_answer);
    ...
    if (answerSubmitted)
        {
            if(answerSubmitted === correctAnswer)
                {
                    correct++;
                    var difficulty = questions[counter].level;
                    if (difficulty === 'easy')
                    {
                        score += 10;
                    }
